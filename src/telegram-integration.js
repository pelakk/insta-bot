...
class TelegramIntegration {
  ...
  async startInstaBotInstance(chatId, instanceId) {
    try {
      ...
      if (id < 1 || id > 3) {
        this.telegramBot.bot.sendMessage(chatId, "❌ Nieprawidłowy ID instancji. Użyj 1, 2 lub 3.");
        return;
      }

      if (this.instances.has(id)) {
        this.telegramBot.bot.sendMessage(chatId, `⚠️ Instancja ${id} jest już uruchomiona.`);
        return;
      }

      this.telegramBot.bot.sendMessage(chatId, `🚀 Uruchamianie InstaBot Instance ${id}...`);
      ...
      // Send status update
      await this.telegramBot.sendStatusUpdate(id, "✅ Uruchomiona pomyślnie", "Bot rozpoczął działanie");

      this.telegramBot.bot.sendMessage(chatId, `✅ InstaBot Instance ${id} uruchomiona pomyślnie!`);

    } catch (error) {
      this.telegramBot.bot.sendMessage(chatId, `❌ Błąd podczas uruchamiania instancji ${instanceId}: ${error.message}`);
  ...
  async stopInstaBotInstance(chatId, instanceId) {
    try {
      ...
      if (!this.instances.has(id)) {
        this.telegramBot.bot.sendMessage(chatId, `⚠️ Instancja ${id} nie jest uruchomiona.`);
        return;
      }

      this.telegramBot.bot.sendMessage(chatId, `🛑 Zatrzymywanie InstaBot Instance ${id}...`);
      ...
      // Send status update
      await this.telegramBot.sendStatusUpdate(id, "🛑 Zatrzymana", "Bot został zatrzymany");

      this.telegramBot.bot.sendMessage(chatId, `✅ InstaBot Instance ${id} zatrzymana pomyślnie!`);

    } catch (error) {
      this.telegramBot.bot.sendMessage(chatId, `❌ Błąd podczas zatrzymywania instancji ${instanceId}: ${error.message}`);
      await this.telegramBot.sendErrorNotification(instanceId, error);
    }
  }
  ...
  async startBotWorkflow(instanceId, instauto, options) {
    try {
      ...
      await this.telegramBot.sendStatusUpdate(instanceId, "🔄 Rozpoczynam pracę", "Bot rozpoczął proces follow/unfollow");
      ...
      if (unfollowedCount > 0) {
        await this.telegramBot.sendStatusUpdate(instanceId, `👋 Unfollowed ${unfollowedCount} users`, "Unfollowed old users");
      ...
      await this.telegramBot.sendStatusUpdate(instanceId, "✅ Praca zakończona", "Bot zakończył dzisiejszą sesję");
      await instauto.sleep(10 * 60 * 1000);
...
*Aktywne:* ${Array.from(this.instances.keys()).length}/3
  ...
  async getRealStats() {
    try {
      ...
      for (let i = 1; i <= 3; i++) {
        try {
          const followedData = await require("fs-extra").readJson(`./followed_${i}.json`);
          const unfollowedData = await require("fs-extra").readJson(`./unfollowed_${i}.json`);
          const likedData = await require("fs-extra").readJson(`./liked-photos_${i}.json`);

          // Count today's actions
          const todayFollowed = followedData.filter(item => 
            todayMs - item.time < dayMs
          ).length;

          const todayUnfollowed = unfollowedData.filter(item => 
            todayMs - item.time < dayMs
          ).length;

          const todayLiked = likedData.filter(item => 
            todayMs - item.time < dayMs
          ).length;
...
*Aktywne instancje:* ${this.instances.size}/3
  ...
  async performUnfollowOld(chatId) {
    try {
      this.telegramBot.bot.sendMessage(chatId, "🔄 Rozpoczynam unfollow starych użytkowników...");
      ...
      for (const [instanceId, instance] of this.instances) {
        if (instance.instauto) {
          try {
            ...
            console.error(`Error unfollowing on instance ${instanceId}:`, error);
          }
        }
      }

      this.telegramBot.bot.sendMessage(chatId, `✅ Unfollow zakończony! Unfollowed ${totalUnfollowed} users total.`);

    } catch (error) {
      this.telegramBot.bot.sendMessage(chatId, `❌ Błąd podczas unfollow: ${error.message}`);
    }
  }
