...
class TelegramBotManager {
  constructor() {
    ...
    this.bot.on("polling_error", (error) => {
      if (
        ...
        console.log(
          "⚠️ Telegram API 409 Conflict detected. This usually means another bot instance is running."
        );
  ...
  setupCommands() {
    ...
    this.bot.onText(/\/start_bot (.+)/, (msg, match) =>
      this.handleStartBot(msg, match[1])
    );
    this.bot.onText(/\/stop_bot (.+)/, (msg, match) =>
      this.handleStopBot(msg, match[1])
    );
    ...
    this.bot.onText(/\/account_help/, (msg) =>
      this.accounts.handleAccountHelp(msg)
    );

    // Unfollow testing commands
    this.bot.onText(/\/test_unfollow_non_mutual (.+)/, (msg, match) =>
      this.handleTestUnfollowNonMutual(msg, match[1])
    );
    this.bot.onText(/\/test_unfollow_all_unknown (.+)/, (msg, match) =>
      this.handleTestUnfollowAllUnknown(msg, match[1])
    );
    this.bot.onText(/\/test_unfollow_old (.+)/, (msg, match) =>
      this.handleTestUnfollowOld(msg, match[1])
    );
    this.bot.onText(/\/test_unfollow_single (.+)/, (msg, match) =>
      this.handleTestUnfollowSingle(msg, match[1])
    );
...
Uwaga: Jeśli masz problemy z 409 Conflict, użyj /kill_all (admin)
  ...
  handleStatus(msg) {
    ...
    const statusList = Array.from(this.instances.entries())
      .map(([userId, instance]) => {
        const account = this.accounts.getAccountByUserId(userId);
        const usersToFollow = account
          ? this.accounts.getUsersToFollowForUser(userId)
          : "Brak";
...
*Konta z targetami:*
${data.accounts.filter((acc) => acc.targets.length > 0).length} / ${
  ...
  async handleStartBot(msg, userIdStr) {
    ...
    if (isNaN(userId)) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Nieprawidłowy user_id. Musi być liczbą."
      );
      return;
    }
    ...
    if (this.instances.has(userId)) {
      this.bot.sendMessage(
        msg.chat.id,
        `⚠️ Bot dla użytkownika ${userId} już działa.`
      );
      return;
    }
    ...
    if (!account) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Konto dla użytkownika ${userId} nie istnieje. Użyj /add_account najpierw.`
      );
      return;
    }
    ...
    if (!account.cookie) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Konto dla użytkownika ${userId} nie ma ustawionego cookie.`
      );
      return;
    }
    ...
    if (!usersToFollow) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Konto dla użytkownika ${userId} nie ma żadnych targetów. Użyj /add_target @username.`
      );
      return;
    }
    ...
    try {
      ...
      this.bot.sendMessage(
        msg.chat.id,
        `✅ Bot dla użytkownika ${userId} został uruchomiony.\n\nUSERS_TO_FOLLOW: ${usersToFollow}`
      );
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas uruchamiania bota dla użytkownika ${userId}: ${error.message}`
      );
    }
  }
  ...
  async handleStopBot(msg, userIdStr) {
    ...
    if (!this.instances.has(userId)) {
      this.bot.sendMessage(
        msg.chat.id,
        `⚠️ Bot dla użytkownika ${userId} nie działa.`
      );
      return;
    }
    ...
    this.bot.sendMessage(
      msg.chat.id,
      `✅ Bot dla użytkownika ${userId} został zatrzymany.`
    );
  }
  ...
  async handleStopAll(msg) {
    ...
    if (userIds.length === 0) {
      this.bot.sendMessage(
        msg.chat.id,
        "⚠️ Brak aktywnych botów do zatrzymania."
      );
      return;
    }
    ...
    this.bot.sendMessage(
      msg.chat.id,
      `✅ Wszystkie boty (${userIds.length}) zostały zatrzymane.`
    );
  }
  ...
  async handleKillAll(msg) {
    ...
    this.bot.sendMessage(
      msg.chat.id,
      `🔄 Zatrzymano wszystkie boty (${userIds.length} instancji) i zrestartowano polling.`
    );
  }
  ...
  async handleStartMyBot(msg) {
    ...
    if (!account) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Nie masz jeszcze skonfigurowanego konta. Skontaktuj się z administratorem."
      );
      return;
    }
    ...
    if (!account.cookie) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Twoje konto nie ma ustawionego cookie. Skontaktuj się z administratorem."
      );
      return;
    }
    ...
    if (!usersToFollow) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Nie masz żadnych targetów. Użyj /add_target @username aby dodać targety do USERS_TO_FOLLOW."
      );
      return;
    }
    ...
    try {
      ...
      this.bot.sendMessage(
        msg.chat.id,
        `✅ Twój bot został uruchomiony!\n\n📝 USERS_TO_FOLLOW: ${usersToFollow}\n\nUżyj /my_status aby sprawdzić status`
      );
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas uruchamiania bota: ${error.message}`
      );
    }
  }
  ...
  handleMyStatus(msg) {
    ...
    if (!account) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Nie masz jeszcze skonfigurowanego konta."
      );
      return;
    }
...
👥 Follow filters: ${userConfig.FOLLOW_USER_MIN_FOLLOWERS}-${
      userConfig.FOLLOW_USER_MAX_FOLLOWERS
    } followers, ${userConfig.FOLLOW_USER_MIN_FOLLOWING}-${
      userConfig.FOLLOW_USER_MAX_FOLLOWING
    } following
...
ADMIN_IDS = [${userId}]
  ...
  async startBotInstance(userId, account, usersToFollow) {
    console.log(
      `🚀 Starting bot for user ${userId} with USERS_TO_FOLLOW: ${usersToFollow}`
    );
    ...
    const instauto = await Instauto(db, browser, {
      ...
      sessionid, // Primary login method
      ...
      followUserMinFollowers:
        parseInt(userConfig.FOLLOW_USER_MIN_FOLLOWERS) || 0,
      followUserMaxFollowers:
        parseInt(userConfig.FOLLOW_USER_MAX_FOLLOWERS) || 10000,
      followUserMinFollowing:
        parseInt(userConfig.FOLLOW_USER_MIN_FOLLOWING) || 0,
      followUserMaxFollowing:
        parseInt(userConfig.FOLLOW_USER_MAX_FOLLOWING) || 5000,
      logger: {
  ...
  async runBotProcess(userId, usersToFollow) {
    try {
      console.log(
        `🎯 Starting process for user ${userId} with targets: ${usersToFollow}`
      );

      // Parse targets
      const usersToFollowArray = usersToFollow
        .split(",")
        .map((t) => t.trim())
        .filter((t) => t);
      ...
      const userConfig = this.accounts.getUserConfig(userId);
      const maxFollowsPerTarget = parseInt(
        userConfig.MAX_FOLLOWS_PER_TARGET || "30"
      );
      const maxFollowersToCollectPerTarget = parseInt(
        userConfig.MAX_FOLLOWERS_TO_COLLECT_PER_TARGET || "100"
      );

      console.log(
        `⚙️ Configuration: ${maxFollowsPerTarget} follows per target, ${maxFollowersToCollectPerTarget} followers to collect per target`
      );
      ...
      try {
        ...
        console.log(
          `📋 Resuming from target index ${currentTargetIndex} (${shuffledTargets[currentTargetIndex]})`
        );
      } catch (error) {
      ...
      console.log(
        `✅ Login status verified, starting to process ${shuffledTargets.length} targets...`
      );
      ...
      for (let i = currentTargetIndex; i < shuffledTargets.length; i++) {
        ...
        try {
          ...
          const targetUserData =
            await instance.instauto.navigateToUserAndGetData(targetUsername);

          if (!targetUserData || targetUserData.id === "unknown") {
            console.log(
              `❌ Could not get user data for ${targetUsername}, skipping...`
            );
            continue;
          }
          ...
          console.log(
            `📥 Collecting followers from ${targetUsername} (max: ${maxFollowersToCollectPerTarget})`
          );
          ...
          try {
            ...
            for await (const followersBatch of instance.instauto.getFollowersOrFollowingGenerator(
              ...
              for (const followerUsername of followersBatch) {
                if (allFollowers.length >= maxFollowersToCollectPerTarget) {
                  console.log(
                    `📊 Reached max followers limit (${maxFollowersToCollectPerTarget}) for ${targetUsername}`
                  );
                  break;
                }
              ...
              if (allFollowers.length >= maxFollowersToCollectPerTarget) {
                console.log(
                  `📊 Stopping collection at ${allFollowers.length} followers for ${targetUsername}`
                );
                break;
              }
            }
            ...
            console.log(
              `⚠️ Error collecting followers from ${targetUsername}: ${error.message}`
            );
          ...
          if (allFollowers.length === 0) {
            console.log(
              `❌ No followers found for ${targetUsername}, skipping...`
            );
            continue;
          }

          console.log(
            `✅ Collected ${allFollowers.length} followers from ${targetUsername} (${pageCount} pages)`
          );

          // Follow each follower with respect to limits and filters
          let followedCount = 0;
          const maxFollowsForThisTarget = Math.min(
            maxFollowsPerTarget,
            allFollowers.length
          );
          ...
          console.log(
            `🎯 Starting to follow ${maxFollowsForThisTarget} followers from ${targetUsername}`
          );
          ...
          for (const followerUsername of allFollowers) {
            try {
              ...
              const prevFollowed =
                instance.db.getPrevFollowedUser(followerUsername);
              if (prevFollowed) {
                console.log(
                  `⏩ Skipped @${followerUsername} – already followed recently`
                );
                continue;
              }
              ...
              // Check hourly limit
              const hourlyFollows =
                instance.instauto.getNumFollowedUsersThisTimeUnit(
                  60 * 60 * 1000
                );
              if (hourlyFollows >= parseInt(userConfig.MAX_FOLLOWS_PER_HOUR)) {
                console.log(
                  `⏸️ Hourly limit reached (${hourlyFollows}/${userConfig.MAX_FOLLOWS_PER_HOUR}), waiting 10 minutes...`
                );
              ...
              if (dailyFollows >= parseInt(userConfig.MAX_FOLLOWS_PER_DAY)) {
                console.log(
                  `⏸️ Daily limit reached (${dailyFollows}/${userConfig.MAX_FOLLOWS_PER_DAY}), stopping...`
                );
                break;
              }
              ...
              const followerData =
                await instance.instauto.navigateToUserAndGetData(
                  followerUsername
                );

              if (!followerData || followerData.id === "unknown") {
                console.log(
                  `⏩ Skipped @${followerUsername} (id: unknown) – could not get user data`
                );
                continue;
              }

              // Apply pre-filtering to remove unwanted users
              const enablePrefilter =
                userConfig.ENABLE_PREFILTER === "true" || true;
              if (enablePrefilter) {
                ...
                if (followerData.is_private) {
                  console.log(
                    `⏩ Skipped @${followerUsername} – private account`
                  );
                  continue;
                }
                ...
                if (!followerData.profile_pic_url) {
                  console.log(
                    `⏩ Skipped @${followerUsername} – no profile picture`
                  );
                  continue;
                }
                ...
                for (const keyword of suspiciousKeywords) {
                  if (username.includes(keyword)) {
                    console.log(
                      `⏩ Skipped @${followerUsername} – suspicious username (${keyword})`
                    );
                    continue;
                  }
                }
                ...
                if (username.length < 3 || username.length > 30) {
                  console.log(
                    `⏩ Skipped @${followerUsername} – username length (${username.length})`
                  );
                  continue;
                }
                ...
                if (
                  ...
                  console.log(
                    `⏩ Skipped @${followerUsername} – invalid username format`
                  );
                  continue;
                }
              }
              ...
              if (
                followerCount <
                parseInt(userConfig.FOLLOW_USER_MIN_FOLLOWERS || 50)
              ) {
                console.log(
                  `⏩ Skipped @${followerUsername} (followers: ${followerCount}, following: ${followingCount}) – doesn't match filters (min followers: ${
                    userConfig.FOLLOW_USER_MIN_FOLLOWERS || 50
                  })`
                );
                continue;
              }
              ...
              if (
                followerCount >
                parseInt(userConfig.FOLLOW_USER_MAX_FOLLOWERS || 5000)
              ) {
                console.log(
                  `⏩ Skipped @${followerUsername} (followers: ${followerCount}, following: ${followingCount}) – doesn't match filters (max followers: ${
                    userConfig.FOLLOW_USER_MAX_FOLLOWERS || 5000
                  })`
                );
                continue;
              }
              ...
              if (
                followingCount <
                parseInt(userConfig.FOLLOW_USER_MIN_FOLLOWING || 50)
              ) {
                console.log(
                  `⏩ Skipped @${followerUsername} (followers: ${followerCount}, following: ${followingCount}) – doesn't match filters (min following: ${
                    userConfig.FOLLOW_USER_MIN_FOLLOWING || 50
                  })`
                );
                continue;
              }
              ...
              if (
                ...
                console.log(
                  `⏩ Skipped @${followerUsername} (followers: ${followerCount}, following: ${followingCount}) – doesn't match filters (max following: ${
                    userConfig.FOLLOW_USER_MAX_FOLLOWING || 5000
                  })`
                );
                continue;
              }
              ...
              if (isPrivate) {
                console.log(
                  `⏩ Skipped @${followerUsername} (followers: ${followerCount}, following: ${followingCount}) – private account`
                );
                continue;
              }
              ...
              try {
                const followResult =
                  await instance.instauto.followUserRespectingRestrictions({
                ...
                if (followResult.success) {
                  ...
                  console.log(
                    `✅ Followed @${username} (id: ${followerData.id}) from target: ${targetUsername}`
                  );
                } else {
                  // Log skip reason
                  const username = followerData.username || "unknown user";
                  console.log(
                    `⏩ Skipped @${username} (id: ${followerData.id}): ${
                      followResult.reason || "unknown reason"
                    }`
                  );
                }
              } catch (followError) {
                console.log(
                  `❌ Failed to follow @${followerUsername} (id: ${followerData.id}): ${followError.message}`
                );
                totalErrorCount++;
              ...
              console.error(
                `❌ Failed to process follower ${followerUsername}: ${error.message}`
              );
              totalErrorCount++;
              // Continue with next follower
            }
          ...
          console.log(
            `✅ Successfully processed target: ${targetUsername} (followed ${followedCount} users)`
          );
          ...
          console.error(
            `❌ Failed to process target ${targetUsername}: ${error.message}`
          );
          totalErrorCount++;
      ...
      const resultMessage =
        `✅ Proces zakończony dla user ${userId}\n\n` +
      ...
      console.log(
        `🎉 Process completed for user ${userId} - Success: ${totalSuccessCount}, Errors: ${totalErrorCount}`
      );
      ...
      console.error(
        `💥 Error in bot process for user ${userId}: ${error.message}`
      );
  ...
  async handleTestUnfollowNonMutual(msg, limitStr) {
    ...
    if (!this.instances.has(userId)) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Bot nie jest uruchomiony. Użyj /start_my_bot najpierw."
      );
      return;
    }
    ...
    try {
      this.bot.sendMessage(
        msg.chat.id,
        `🔄 Rozpoczynam unfollow non-mutual followers (limit: ${limit})...`
      );

      const result = await instance.instauto.unfollowNonMutualFollowers({
        limit,
      });

      this.bot.sendMessage(
        msg.chat.id,
        `✅ Unfollow non-mutual zakończony! Unfollowed: ${result} users`
      );
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas unfollow non-mutual: ${error.message}`
      );
    }
  }
  ...
  async handleTestUnfollowAllUnknown(msg, limitStr) {
    ...
    try {
      this.bot.sendMessage(
        msg.chat.id,
        `🔄 Rozpoczynam unfollow all unknown (limit: ${limit})...`
      );
      ...
      this.bot.sendMessage(
        msg.chat.id,
        `✅ Unfollow all unknown zakończony! Unfollowed: ${result} users`
      );
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas unfollow all unknown: ${error.message}`
      );
    }
  }
  ...
  async handleTestUnfollowOld(msg, params) {
    ...
    if (!ageInDays) {
      this.bot.sendMessage(
        msg.chat.id,
        "❌ Użycie: /test_unfollow_old <dni> [limit]\nPrzykład: /test_unfollow_old 7 10"
      );
      return;
    }
    ...
    try {
      this.bot.sendMessage(
        msg.chat.id,
        `🔄 Rozpoczynam unfollow old followed (${ageInDays} dni, limit: ${
          limit || "bez limitu"
        })...`
      );

      const result = await instance.instauto.unfollowOldFollowed({
        ageInDays,
        limit,
      });
      ...
      this.bot.sendMessage(
        msg.chat.id,
        `✅ Unfollow old followed zakończony! Unfollowed: ${result} users`
      );
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas unfollow old followed: ${error.message}`
      );
    }
  }
  ...
  async handleTestUnfollowSingle(msg, username) {
    ...
    try {
      this.bot.sendMessage(
        msg.chat.id,
        `🔄 Rozpoczynam unfollow pojedynczego użytkownika: @${username}...`
      );
      ...
      if (result.noActionTaken) {
        this.bot.sendMessage(
          msg.chat.id,
          `ℹ️ Użytkownik @${username} już nie jest followowany lub nie znaleziono przycisku unfollow`
        );
      } else {
        this.bot.sendMessage(
          msg.chat.id,
          `✅ Pomyślnie unfollowed @${username}`
        );
      }
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas unfollow @${username}: ${error.message}`
      );
    }
  }
  ...
  async handleClearProgress(msg) {
    ...
    try {
      ...
      this.bot.sendMessage(
        msg.chat.id,
        "🗑️ Postęp został wyczyszczony. Następne uruchomienie bota zacznie od początku."
      );
    } catch (error) {
      this.bot.sendMessage(
        msg.chat.id,
        `❌ Błąd podczas czyszczenia postępu: ${error.message}`
      );
    }
  }
}
...
botManager.bot.on("polling_error", (error) => {
  if (error.code === "ETELEGRAM" && error.message.includes("409 Conflict")) {
    conflictRetryCount++;
    console.log(
      `⚠️ Telegram API 409 Conflict detected (attempt ${conflictRetryCount}/${maxConflictRetries})`
    );
